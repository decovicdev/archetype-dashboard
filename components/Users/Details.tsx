...
const Component = () => {
  ...
  const handleAssignSubscription = async (id: string) => {
    try {
      await user.assignSandboxSubscription({
  ...
  const handleCreatePaymentLink = async (id: string) => {
    try {
      const result = await user.generatePaymentLink({
        custom_uid: router.query.userId as string,
        tier_id: id
      });
  ...
  const handleSendInvoice = async (id: string) => {
    ...
    try {
      await user.generatePaymentLinkEmail({
        custom_uid: router.query.userId as string,
        tier_id: id
      });
    ...
    <div className="users-details-page">
      <div className="flex space-x-2 items-center">
        ...
        <div className="flex flex-row gap-1">
          <Button
            url={`${ROUTES.USERS.EDIT}/${data.custom_uid}`}
        ...
        <table className="border-collapse table-auto w-full">
          <tbody>
            ...
            <tr>
              ...
              <td className="p-1 link">
                {data.tier_id && (
                  <Link href={`/products/${data.tier_id}`}>{data.tier_id}</Link>
                )}
              </td>
            </tr>
            <tr>
        ...
        <tbody>
          {(tiers as Tier[]).map((tier) => (
            <tr key={tier.tier_id} className="border">
              ...
              <td className="p-1">
                {tier.quota ? `${tier.quota}/day` : `Unlimited`}
              </td>
              <td className="p-1">
                <DropdownMenu
                ...
                >
                  <Button
                    className="w-full mb-2"
                    onClick={() => {
                      handleAssignSubscription(tier.tier_id);
                    }}
                  >
                    Assign
                  </Button>
                  <Button
                    className="w-full mb-2"
                    onClick={() => {
                      handleCreatePaymentLink(tier.tier_id);
                    }}
                  >
                    Payment link
                  </Button>
